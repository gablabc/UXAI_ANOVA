...
R = get_ANOVA_1(X, h_add)
h_preds = R[0, :, 0]
R = R + h_preds.reshape((1, -1, 1))
R = R[..., 1:]  # (N, N, d) tensor s.t. R_ijk = h_(r_{k}(x^(j), x^(i)))
R_c = R - R.mean(0, keepdims=True)
...
for k in range(2):
    ...
    plt.figure()
    for n in range(6):
        plt.plot(X[argsort_x_i, k], R[:, n, k][argsort_x_i], alpha=0.5, linewidth=4)
    plt.ylim(-4, 4)
    plt.xlim(-1, 1)
    plt.xlabel(r"$x_k$")
    ...
    plt.figure()
    for n in range(6):
        plt.plot(X[argsort_x_i, k], R_c[:, n, k][argsort_x_i], alpha=0.5, linewidth=4)
    # Show the mean centered PDP
    plt.plot(X[argsort_x_i, k], np.mean(R_c[..., k], 1)[argsort_x_i], "k--", linewidth=6)
    plt.ylim(-4, 4)
    plt.xlim(-1, 1)
    plt.xlabel(r"$x_k$")
